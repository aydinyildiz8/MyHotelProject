...
namespace HotelProject.WebUI.Controllers
{
    public class TestimonialAdminController : Controller
    {
        ...
        public async Task<IActionResult> AddTestimonial(TestimonialViewModel model)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:32685/api/Testimonial", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:32685/api/Testimonial/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateTestimonial(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:32685/api/Testimonial/{id}");
        ...
        public async Task<IActionResult> UpdateTestimonial(TestimonialViewModel model)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(model);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:32685/api/Testimonial/", stringContent);
